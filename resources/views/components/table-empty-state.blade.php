...
JS;
    $defaultTitleExpr = "{$resolveLabel}(({$loadingExpr}) ? ".json_encode($messageLoading).' : (({$filtersExpr}) ? '.json_encode($messageFiltered).' : (({$customEmptyExpr}) || '.json_encode($messageEmpty).')))';
    $defaultSubtitleExpr = "{$resolveLabel}(({$loadingExpr}) ? ".json_encode($subtitleLoading).' : (({$filtersExpr}) ? '.json_encode($subtitleFiltered).' : '.json_encode($subtitleEmpty).'))';
    $titleExpr = $messageExpr ?? $defaultTitleExpr;
    $bodySubtitleExpr = $subtitleExpr ?? $defaultSubtitleExpr;
    $hideSubtitleExpr = $customEmptyExpr !== 'null' && ! $subtitleExpr
        ? "!({$customEmptyExpr})"
        : 'true';
@endphp
...
<div class="nttu-table-empty-state py-2">
  <div
    class="mx-auto max-w-lg rounded-lg border border-dashed px-4 py-6"
    :class="({{ $loadingExpr }})
  ...
  >
    <svg
      ...
      :class="({{ $loadingExpr }}) ? 'text-[var(--nttu-primary)] animate-pulse' : (({{ $filtersExpr }}) ? 'text-orange-400' : 'text-gray-300')"
      fill="none"
      stroke="currentColor"
      viewBox="0 0 24 24"
      stroke-width="2"
      ...
      <template x-if="({{ $filtersExpr }}) && !({{ $loadingExpr }})">
        <p class="mt-3 text-sm">
          ...
          >
            ...
            <span x-text="{{ $resolveLabel }}('Xóa tất cả bộ lọc')"></span>
          </button>
        </p>
      </template>
    @endif
